...
class EmailModel extends CI_Model
{
...

    {
            ...
            $query = $this->db->query("SELECT e.* , e_r.* , (SELECT CONCAT_WS(' ' ,first_name ,middle_initial ,last_name) from `profile` where user_id = e_r.sender_id) as receiver from emails e
                                        LEFT JOIN `profile` p on e.user_id = p.user_id
                                        INNER JOIN emails_receiver e_r on e.id = e_r.email_id
                                        where e_r.receiver_is_delete = 'NO' and e_r.receiver_id = $user_id order by e.id DESC
                                        ");
...
INNER JOIN emails_receiver e_r on e.id = e_r.email_id
where  e_r.sender_is_delete = 'NO'  and e_r.sender_id = $user_id order by e.id DESC");
...
where e_r.trash_is_delete = 'NO' and (e_r.sender_is_delete = 'YES' or receiver_is_delete = 'YES') and (e_r.receiver_id = $user_id or e_r.sender_id = $user_id) order by e.date ASC");
    ...
    {
        $query = $this->db->query("SELECT e.* , e_r.* , (SELECT CONCAT_WS(' ' ,first_name ,middle_initial ,last_name) from `profile` where user_id = e_r.sender_id) as sender ,(SELECT CONCAT_WS(' ' ,first_name ,middle_initial ,last_name) from `profile` where user_id = e_r.receiver_id) as receiver  from emails e
    ...
    function insertEmails($status, $receiver_ids)

    {
        ...
        if($receiver_ids[0] == null){
            $data = array(
                'email_id' => $insert_id,
                'receiver_name' => EMAILSENDER,
                'sender_id' => $this->session->userdata(USER_ID)?$this->session->userdata(USER_ID):'',
                'senderEmail' => $this->input->post('email')?$this->input->post('email'):'',
            );
...
where e_r.receiver_is_delete = 'NO' and e_r.receiver_id = $user_id  and e_r.display = 'unseen' order by e.date ASC");
    ...
    function changedToSeen($mail_id)
    {
        $query = $this->db->query("update emails_receiver set display = 'seen' where email_id = $mail_id");
        if ($this->db->affected_rows() > 0) {
            return TRUE;
        }
        return false;
    ...
    function getAllMail($userId)
        {

            $query = $this->db->query("SELECT e.* ,  er.* ,GROUP_CONCAT(CONCAT_WS(' ' ,p.first_name ,p.middle_initial ,p.last_name)) receiverName,
                                        (SELECT CONCAT_WS(' ' ,first_name ,middle_initial ,last_name) FROM profile WHERE user_id = er.sender_id) senderName
                                        FROM emails e
                                        INNER JOIN emails_receiver er ON e.id = er.email_id
                                        INNER JOIN profile p ON er.receiver_id = p.user_id
                                        WHERE  er.sender_is_delete = 'NO' AND er.sender_id = $userId
                                        GROUP BY e.id
                                        ORDER BY e.id DESC
                                        ");
