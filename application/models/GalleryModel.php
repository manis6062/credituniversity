...
class GalleryModel extends CI_Model {
    ...
    function getPhotoDetails($id) {
        $query = $this -> db -> get_where('nc_gallery', array('gallery_id' => $id));
    ...
    function update($user_id, $ph) {
        ...
        $data = array('gallery_title' => $this -> input -> post('gallery_title'), 'album_id' => $this -> input -> post('album_id'), 'gallery_path' => $ph, 'updt_dt' => $today, 'updt_by' => $this -> session -> userdata(USER_ID));
    ...
    function insert($ph) {
        $today = date("Y-m-d H:i:s");
        $data = array('gallery_title' => $this -> input -> post('gallery_title'), 'album_id' => $this -> input -> post('album_id'), 'gallery_path' => $ph, 'crtd_dt' => $today, 'crtd_by' => $this -> session -> userdata(USER_ID), 'updt_dt' => $today, 'updt_by' => $this -> session -> userdata(USER_ID));
        $this -> db -> insert('nc_gallery', $data);
        $userid = $this -> db -> insert_id();
        return $userid;
    }
    
    function checkGallery($album_id){
        $query = $this -> db -> get_where('nc_gallery', array('album_id' => $album_id));
